    ...
    <section className="relative overflow-hidden rounded-2xl border border-white/15 bg-gradient-to-br from-white/[0.09] via-white/[0.04] to-transparent p-5 shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-2xl">
      ...
      <div className="relative mt-4 space-y-3">
        {zones.map((item, index) => {
          ...
          return (
            <div key={item.zone} className="rounded-xl border border-white/10 bg-white/[0.05] p-4 shadow-inner ring-1 ring-white/10 backdrop-blur-md transition-colors hover:border-white/20">
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {item.countries.split(', ').map((c) => (
                  <span key={c} className="rounded-full border border-white/10 bg-white/[0.06] px-2 py-1 text-[10px] font-bold text-slate-200">{c}</span>
                ))}
              </div>
            </div>
          );
