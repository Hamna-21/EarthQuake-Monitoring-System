...
export default function GlobeHoverTooltip({ event, position, above = false }: { event: Earthquake; position: { x: number; y: number }; above?: boolean }) {
  return <div className={`pointer-events-none absolute z-40 w-60 -translate-x-1/2 ${above ? '-translate-y-full' : 'translate-y-4'} rounded-xl border border-cyan-200/25 bg-slate-950/95 px-3 py-2.5 font-serif text-white shadow-2xl backdrop-blur-xl`} style={{ left: position.x, top: position.y }}>
