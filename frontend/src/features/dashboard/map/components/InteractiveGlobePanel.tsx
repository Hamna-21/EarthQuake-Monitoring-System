...
export default function InteractiveGlobePanel({ events, onSelect, onDetails, autoRotate = true, focusLocation, focusLabel, popupMode = 'compact' }: { events: Earthquake[]; onSelect: (event: Earthquake) => void; onDetails: (event: Earthquake) => void; autoRotate?: boolean; focusLocation?: { lat: number; lng: number; altitude?: number } | null; focusLabel?: string; popupMode?: 'compact' | 'historical' }) {
  ...
  const strongestId = useMemo(() => points.reduce<string | null>((id, event) => !id || event.magnitude > (points.find((item) => item.id === id)?.magnitude ?? -1) ? event.id : id, null), [points]);
  ...
  const pulseEvents = useMemo(() => globePoints.filter((event) => event.magnitude >= 6), [globePoints]);
  const eventFocus = useMemo(() => clusterFocus(points), [points]);
  const activeFocus = eventFocus ?? focusLocation;
  ...
  useEffect(() => setupControls(globe.current?.controls?.(), shouldRotate), [width, shouldRotate]);
  useEffect(() => { if (globeReady && activeFocus) { const controls = globe.current?.controls?.(); if (controls) controls.autoRotate = false; globe.current?.pointOfView?.({ lat: activeFocus.lat, lng: activeFocus.lng, altitude: activeFocus.altitude ?? 1.05 }, 1100); } }, [globeReady, activeFocus?.lat, activeFocus?.lng, activeFocus?.altitude]);
  useEffect(() => { setHoveredId(null); setHoverPosition({ x: 0, y: 0 }); }, [points]);
  useEffect(() => () => { if (hoverTimer.current) window.clearTimeout(hoverTimer.current); if (closeTimer.current) window.clearTimeout(closeTimer.current); }, []);
  ...
  const focusOnEvent = (event: Earthquake) => { if (closeTimer.current) window.clearTimeout(closeTimer.current); const controls = globe.current?.controls?.(); if (controls) controls.autoRotate = false; const screen = screenPosition(event); if (screen) setPopupPosition(screen); setClosing(false); setSelected(event); onSelect(event); globe.current?.pointOfView?.({ lat: event.latitude, lng: event.longitude, altitude: 0.86 }, 700); requestAnimationFrame(() => syncPopup(event)); };
  const syncHover = (event: Earthquake) => { const screen = screenPosition(event); if (screen) setHoverPosition(screen); };
  const handleHover = (event: Earthquake | null) => { if (hoverTimer.current) window.clearTimeout(hoverTimer.current); if (event) { const id = event.id; hoverTimer.current = window.setTimeout(() => { setHoveredId((current) => current === id ? current : id); syncHover(event); }, 90); return; } hoverTimer.current = window.setTimeout(() => setHoveredId(null), 150); };
...
function setupControls(controls: any, autoRotate: boolean) { if (!controls) return; controls.autoRotate = autoRotate; controls.autoRotateSpeed = 0.2; controls.enableDamping = true; controls.dampingFactor = 0.08; controls.minDistance = 90; controls.maxDistance = 500; }
...
function clusterFocus(events: Earthquake[]) {
  if (!events.length) return null;
  const lat = events.reduce((sum, event) => sum + event.latitude, 0) / events.length;
  const lng = events.reduce((sum, event) => sum + event.longitude, 0) / events.length;
  const latSpread = Math.max(...events.map((event) => event.latitude)) - Math.min(...events.map((event) => event.latitude));
  const lngSpread = Math.max(...events.map((event) => event.longitude)) - Math.min(...events.map((event) => event.longitude));
  const span = Math.max(latSpread, lngSpread);
  const altitude = span > 80 ? 1.42 : span > 35 ? 1.18 : span > 12 ? 0.98 : span > 4 ? 0.82 : 0.68;
...
function GlobeLegend() {
  ...
  return <div className="pointer-events-none absolute bottom-3 left-3 z-20 flex max-w-[calc(100%-1.5rem)] flex-wrap gap-2 rounded-xl border border-white/10 bg-slate-950/80 px-3 py-2 text-[10px] font-bold text-slate-300 shadow-xl backdrop-blur-xl">
