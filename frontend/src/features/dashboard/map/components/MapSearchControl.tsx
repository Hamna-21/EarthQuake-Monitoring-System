...
/** Renders the map's place search and reports submission/clear actions to useMapControls. */
    ...
    <Section label="Find a location" icon={<Search className="h-3.5 w-3.5 text-cyan-400" />} tint="from-cyan-500/10 to-fuchsia-500/10" border="border-cyan-400/20" className="xl:col-span-2">
      <div className="flex gap-2">
        <label className="relative block flex-1">
          <Input prefix={<Search className="h-4 w-4 text-cyan-400" />} value={props.query} onChange={(e) => props.onQuery(e.target.value)} onPressEnter={props.onSearchSubmit} placeholder="Search a place..." className="map-search-input" />
        ...
        <Button onClick={props.onSearchSubmit} disabled={props.isSearching || !props.query.trim()} className="map-search-submit" title="Fly to place">{props.isSearching ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}</Button>
        <Button onClick={props.onLocateMe} disabled={props.isLocating} className="map-search-locate">{props.isLocating ? <Loader2 className="h-4 w-4 animate-spin" /> : <LocateFixed className="h-4 w-4" />}</Button>
      </div>
...
/** Renders or coordinates inline error for this frontend module. */
