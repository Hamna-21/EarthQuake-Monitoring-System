...
interface MapCanvasProps {
  ...
  popupMode?: 'compact' | 'historical'; focusTarget?: FlyTarget | null;
}
    ...
    <div className="h-[420px] sm:h-[500px] lg:h-[640px]">
      <div className="relative h-full w-full overflow-hidden rounded-2xl border border-white/10 bg-slate-950 shadow-2xl [&_.leaflet-container]:!overflow-visible [&_.leaflet-pane]:!overflow-visible [&_.leaflet-popup-pane]:!z-[1000]">
        <MapContainer center={[20, 0]} zoom={2} style={{ height: '100%', width: '100%' }} scrollWheelZoom>
          ...
          <ResizeMapOnContainerChange /><FitBounds events={mappableEvents.slice(0, 80)} /><FlyToSelected event={selectedEvent} />
          ...
          <MapEventLayers events={mappableEvents} strongestIds={strongestIds} heat={Boolean(props.heat) || mappableEvents.length > 500} onSelect={props.onSelect} onDetails={props.onDetails} popupMode={props.popupMode} />
        </MapContainer>
        <MapLegend />
