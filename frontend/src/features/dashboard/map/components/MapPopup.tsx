...
function splitDateTime(time: string) {
  ...
  return {
    date: new Intl.DateTimeFormat('en', { month: 'short', day: '2-digit', year: 'numeric', timeZone: 'UTC' }).format(value),
    clock: `${new Intl.DateTimeFormat('en', { hour: '2-digit', minute: '2-digit', timeZone: 'UTC' }).format(value)} UTC`,
  };
}
...
export default function MapPopup({ event, isStrongest = false, onSelect, onDetails, mode }: MapPopupProps) {
  ...
  return (
    <article className={`w-[190px] max-w-[72vw] rounded-lg border ${isStrongest ? 'border-red-400/45' : 'border-cyan-500/20'} bg-[#071321] p-2 font-serif text-white shadow-2xl shadow-black/40`}>
      {isStrongest && <div className="mb-1.5 rounded-md bg-red-500/15 px-2 py-0.5 text-[7px] font-black uppercase tracking-[0.1em] text-red-200">Strongest in View</div>}
      <p className="text-[8px] font-black uppercase tracking-[0.2em] text-cyan-300/70">Location</p>
      <h3 className="mt-1 line-clamp-2 min-h-[30px] text-[11px] font-black leading-snug text-slate-100">{event.place || 'Unknown location'}</h3>
      <p className="mt-1 text-[10px] font-bold text-cyan-100">{countryOf(event.place)}</p>
      <div className="mt-2 space-y-1.5">
        <InfoRow icon={<Gauge />} label="Magnitude"><span className="rounded-full bg-teal-400 px-2 py-0.5 text-[10px] font-black text-white shadow-sm shadow-teal-900/40">M {event.magnitude.toFixed(1)}</span></InfoRow>
        <InfoRow icon={<CalendarDays />} label="Date"><span>{date}</span></InfoRow>
        <InfoRow icon={<Clock3 />} label="Time"><span>{clock}</span></InfoRow>
      </div>
      {onDetails && <button type="button" onClick={openDetails} className="mt-2 w-full rounded-md bg-teal-400 px-3 py-1.5 text-[9px] font-black text-white shadow-lg shadow-teal-950/40 transition hover:bg-teal-300 focus:outline-none focus:ring-2 focus:ring-teal-200">View Details</button>}
    </article>
  );
}
