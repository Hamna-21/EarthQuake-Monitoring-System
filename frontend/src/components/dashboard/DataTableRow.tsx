...
export default function DataTableRow({ event, index, onSelect }: DataTableRowProps) {
  const accent = tierAccent(event.magnitude);

  return (
    <tr
      onClick={() => onSelect(event)}
      className={`cursor-pointer transition hover:bg-gradient-to-r hover:from-cyan-500/10 hover:to-violet-500/10 ${index % 2 ? 'bg-white/[0.04]' : 'bg-white/[0.02]'}`}
    >
      <td className={`border-l-4 px-6 py-4 ${accent.border}`}>
        <span className={`inline-flex items-baseline gap-1 rounded-2xl px-2 py-1 ${accent.chip}`}>
          <span className={`font-serif text-xl font-black italic ${accent.text}`}>{event.magnitude.toFixed(1)}</span>
      ...
      <td className="px-6 py-4"><span className="font-mono text-xs font-semibold text-violet-600">{fmtDate(event.time, 'UTC')}</span></td>
