...
export default function OverviewStatGrid({ earthquakes }: { earthquakes: Earthquake[] }) {
  ...
  const cards = [
    ['Recorded Today', 'seismic events worldwide', stats.today, Globe2, 'from-cyan-500 to-blue-500', 'shadow-cyan-500/20', 'neutral', 'Live', 3],
    ['Average Magnitude', 'across today’s activity', stats.avgMag.toFixed(1), TrendingUp, 'from-emerald-500 to-teal-400', 'shadow-emerald-500/20', 'positive', 'Stable', 5],
  ...
  return <section><div className="mb-4 flex items-baseline justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-red-700">Situation Room</p><h2 className="mt-1 font-serif text-xl font-black tracking-tight text-white">Global Pulse, at a Glance</h2></div></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{cards.map(([label, caption, value, icon, gradient, glow, tone, tag, seed]) => <OverviewStatCard key={label} label={label} caption={caption} value={value} icon={icon} gradient={gradient} glow={glow} toneChip={toneStyles[tone as CardTone].chip} tag={tag} bars={spark(seed)} />)}</div></section>;
}
