...
export default function AlertMatches({ matches, onOpen }: AlertMatchesProps) {
  if (!matches.length) {
    return (
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {matches.map((event) => (
        <article key={event.id} onClick={() => onOpen(event)} className="cursor-pointer rounded-2xl border border-white/10 bg-white/[0.06] p-4 shadow-xl backdrop-blur-xl transition hover:-translate-y-0.5 hover:border-cyan-200/30">
