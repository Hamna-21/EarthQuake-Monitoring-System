    ...
    <section className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_340px]">
        <section className="grid gap-4">
          {events.length > 0 ? events.slice(0, 10).map((event) => <LiveFeedCard key={event.id} event={event} onDetails={(selected) => select(selected.id)} />) : <div className="rounded-2xl border border-white/12 bg-white/[0.07] p-8 text-center shadow-sm backdrop-blur"><p className="text-sm font-semibold text-slate-400">No events match your current filters.</p></div>}
        </section>
