...
import HistoricalChartCard from './HistoricalChartCard';
import { ChartTooltip, axis, grid, labelStyle, margin } from './HistoricalRechartBase';
import { fmt, peakRow, tickYears } from './historicalChartData';
...
export default function HistoricalYearlyChart({ rows }: { rows: HistoricalRow[] }) {
  ...
  return (
    <HistoricalChartCard title="Earthquake Frequency by Year" subtitle="Yearly earthquake counts across the selected Pakistan-area range." insight={`The highest activity occurred in ${peak?.year ?? 'the selected range'} with ${fmt(peak?.count ?? 0)} earthquakes.`}>
