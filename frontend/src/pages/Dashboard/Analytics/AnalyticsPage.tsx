...
export default function AnalyticsPage({ earthquakes, isLoading, dataError, lastUpdated, globalSearch = '' }: DashboardProps) {
  ...
  const major = significant(events).filter((e) => e.magnitude >= 5).length;
    ...
    <>
      ...
      <section className="mb-6 mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <AnalyticsStatCard
          ...
          help="Typical strength of the loaded earthquakes"
          gradient="from-cyan-400 via-sky-500 to-blue-600"
          glow="shadow-cyan-900/40"
        />
        <AnalyticsStatCard
          icon={<AlertTriangle className="h-4 w-4" />}
          label="Stronger Earthquakes"
          value={String(major)}
          help="Reports that may need more attention"
          gradient="from-amber-400 via-orange-500 to-rose-500"
          glow="shadow-orange-900/40"
        />
        <AnalyticsStatCard
          icon={<ShieldCheck className="h-4 w-4" />}
          label="Checked Reports"
          value={String(stats.reviewed)}
          help="Reports marked as reviewed"
        ...
        <AnalyticsStatCard
          ...
          help="Farthest below the surface"
          gradient="from-violet-400 via-purple-500 to-fuchsia-600"
          glow="shadow-purple-900/40"
        />
        <AnalyticsStatCard
          icon={<Activity className="h-4 w-4" />}
          label="Highest Alerts"
          value={String(stats.red)}
          help="Most serious alert records"
      ...
      <div className="mb-4 mt-8">
        <p className="font-serif text-[10px] font-black uppercase tracking-[0.24em] text-cyan-200">Realtime visual analysis</p>
        <h2 className="mt-1 font-serif text-2xl font-black text-white">Live earthquake graphs</h2>
      </div>
      <section className="grid gap-6 xl:grid-cols-2">
        ...
        <MagnitudeDepthChart events={events} />
      </section>
    </>
  );
}
