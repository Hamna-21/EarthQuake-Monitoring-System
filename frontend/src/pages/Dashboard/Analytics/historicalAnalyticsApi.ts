...
export type HistoricalAnalyticsResponse = {
  ...
  metadata: { generatedAt: string; documentCount: number; geographicClassification: string; pointInPolygonApplied: boolean };
};
...
export function mapHistoricalEvents(events: HistoricalMapEvent[]): Earthquake[] {
  return events.flatMap((event) => {
    ...
    if (!Number.isFinite(longitude) || !Number.isFinite(latitude) || !Number.isFinite(event.magnitude) || !Number.isFinite(event.depth)) return [];
    return [{
      id: event.usgsId, magnitude: event.magnitude!, place: event.place, latitude, longitude, depth: event.depth!,
